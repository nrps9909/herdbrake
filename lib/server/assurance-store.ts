...
export async function getLatestStoredRun() {
  const latest = await getDatabase()
    .prepare('SELECT id FROM stress_runs ORDER BY created_at DESC LIMIT 1')
    .first<{ id: string }>();
  return latest ? getStoredRun(latest.id) : null;
}

export async function createStressRun(input: {
  scenarioId: ScenarioId;
  severity: number;
  liquidityFloor: number;
}): Promise<StoredRun> {
  validateScenario(input.scenarioId);
  validateSeverity(input.severity);
  validateFloor(input.liquidityFloor);
  ...
  const auditInputs = [
    ...
    {
      type: risk.state === 'CRITICAL' ? 'BREAKER_TRIGGERED' : 'BATCH_REVIEWED',
      detail: {
        state: risk.state,
  ...
  const statements: D1PreparedStatement[] = [
    database
      .prepare(
        `INSERT INTO stress_runs (id, scenario_id, severity, liquidity_floor, state, reason_code, directional_agreement, destination_concentration, proposed_outflow, projected_buffer, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .bind(
        runId,
        input.scenarioId,
  ...
  for (const intent of risk.intents)
    statements.push(
      database
        .prepare(
  ...
  for (const audit of audits)
    statements.push(
      database
        .prepare(
          `INSERT INTO audit_events (id, run_id, event_type, detail_json, previous_hash, event_hash, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        )
        .bind(
          audit.id,
          runId,
  ...
  return {
    ...
    risk,
    commitments,
    auditHead: audits.at(-1)?.eventHash ?? null,
  };
}
...
export async function getStoredRun(runId: string): Promise<StoredRun | null> {
  ...
  const run = await database
    .prepare('SELECT * FROM stress_runs WHERE id = ?')
    .bind(runId)
    .first<RunRow>();
  if (!run) return null;
  const rows = await database
    .prepare(
      'SELECT * FROM payment_intents WHERE run_id = ? ORDER BY intent_id',
    )
    .bind(runId)
    .all<IntentRow>();
  const intents = rows.results.map(toIntent);
  const releasedIds = intents
    .filter((item) => item.status === 'RELEASED')
    .map((item) => item.id);
  const risk = runRiskEngine({
    scenarioId: run.scenario_id,
    severity: run.severity,
    liquidityFloor: run.liquidity_floor,
    releasedIds,
  });
  const head = await database
    .prepare(
      'SELECT event_hash FROM audit_events WHERE run_id = ? ORDER BY created_at DESC, id DESC LIMIT 1',
    )
    .bind(runId)
  ...
  return {
    ...
    commitments: Object.fromEntries(
      rows.results.map((row) => [row.intent_id, row.commitment]),
    ),
    auditHead: head?.event_hash ?? null,
  };
}

export async function releaseIntents(
  runId: string,
  input: {
    count: number;
  ...
  if (!input.confirmed)
    throw new InputError('Explicit human confirmation is required.');
  const authorizationReason = input.authorizationReason.trim();
  if (authorizationReason.length < 8 || authorizationReason.length > 160)
  ...
  const replay = await database
    .prepare(
      'SELECT response_json FROM idempotency_keys WHERE run_id = ? AND key = ?',
    )
    .bind(runId, input.idempotencyKey)
    .first<{ response_json: string }>();
  if (replay)
    return {
      ...(JSON.parse(replay.response_json) as object),
      idempotentReplay: true,
    };
  const stored = await getStoredRun(runId);
  if (!stored) throw new NotFoundError('Stress run not found.');
  const held = stored.risk.intents.filter((item) => item.status === 'HELD');
  const ids = input.prioritizeCritical
    ? selectSafeRelease(held, input.count)
    : held.slice(0, input.count).map((item) => item.id);
  const releasedAt = new Date().toISOString();
  const previousHash = stored.auditHead;
  const detail = {
    releasedIntentIds: ids,
    prioritizeCritical: input.prioritizeCritical,
    authorization: 'human-confirmed',
    authorizationReason,
    remainingHeld: Math.max(0, held.length - ids.length),
  };
  ...
  statements.push(
    database
      .prepare(
        'INSERT INTO idempotency_keys (key, run_id, operation, response_json, created_at) VALUES (?, ?, ?, ?, ?)',
      )
      .bind(
  ...
  statements.push(
    database
      .prepare('UPDATE stress_runs SET updated_at = ? WHERE id = ?')
      .bind(releasedAt, runId),
  );
  await database.batch(statements);
  return response;
}
...
export async function getEvidence(runId: string) {
  ...
  const chainValid = await verifyAuditChain(runId, audits);
  const evidence = {
    schemaVersion: '1.0',
    generatedAt: new Date().toISOString(),
    product: 'HerdBrake Taiwan',
    run: stored,
    audit: { chainValid, events: audits },
    controls: {
      custody: false,
...
async function buildAuditChain(
  runId: string,
  inputs: Array<{ type: string; detail: unknown }>,
  timestamp: string,
) {
  const events: Awaited<ReturnType<typeof buildAuditEvent>>[] = [];
  let previous: string | null = null;
  for (let index = 0; index < inputs.length; index += 1) {
    const createdAt = new Date(
      new Date(timestamp).getTime() + index,
    ).toISOString();
...
async function buildAuditEvent(
  ...
  return makeAuditEvent(runId, eventType, detail, previousHash, createdAt);
}
...
function validateScenario(value: string): asserts value is ScenarioId {
  if (!scenarios.some((item) => item.id === value))
    throw new InputError('Unknown scenario.');
}
function validateSeverity(value: number) {
  if (!Number.isFinite(value) || value < 0.1 || value > 1)
    throw new InputError('Severity must be between 0.1 and 1.');
}
function validateFloor(value: number) {
  if (!Number.isInteger(value) || value < 50 || value > 95)
    throw new InputError('Liquidity floor must be an integer from 50 to 95.');
}
function validateCount(value: number) {
  if (!Number.isInteger(value) || value < 1 || value > 10)
    throw new InputError('Release count must be an integer from 1 to 10.');
}
function validateRunId(value: string) {
  if (!/^RUN-[0-9a-f-]{36}$/.test(value))
    throw new InputError('Invalid run identifier.');
}
