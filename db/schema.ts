...
export const stressRuns = sqliteTable(
  'stress_runs',
  {
    ...
    intentCount: integer('intent_count').notNull().default(30),
  ...
  (table) => [
    index('idx_stress_runs_created_at').on(table.createdAt),
    index('idx_runs_owner_created').on(table.ownerId, table.createdAt),
  ],
);
...
export const paymentIntents = sqliteTable(
  'payment_intents',
  {
    runId: text('run_id')
      .notNull()
      .references(() => stressRuns.id, { onDelete: 'cascade' }),
...
export const auditEvents = sqliteTable(
  ...
  (table) => [
    index('idx_audit_events_run_created').on(table.runId, table.createdAt),
    uniqueIndex('idx_audit_events_run_sequence').on(
      table.runId,
      table.sequence,
    ),
  ],
...
export const policyVersions = sqliteTable(
  ...
  (table) => [primaryKey({ columns: [table.ownerId, table.revision] })],
);
export const requestLimits = sqliteTable(
  ...
  (table) => [primaryKey({ columns: [table.ownerId, table.window] })],
);
