...
import {
  ...
  Building2, ClipboardCheck, Cpu, Download, FileCheck2, Fingerprint, Gauge,
  History, LockKeyhole, Network, Pause, Play, Radar, RotateCcw,
  Scale, Search, ShieldCheck, SlidersHorizontal, TimerReset, X,
} from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => { runScenarioRef.current = runScenario; stageReleaseRef.current = stageRelease; riskRef.current = risk; }, [risk, runScenario, stageRelease]);
...
function StartupSite({ onLaunch }: { onLaunch: () => void }) {
  return <main className="startup-site min-h-screen bg-[#f4f3ee] text-[#111714]">
    ...
    <section id="top" className="overflow-hidden border-b border-[#cfd3cb]">
      <div className="startup-container grid min-h-[720px] items-center gap-14 py-20 lg:grid-cols-[.88fr_1.12fr] lg:py-24">
        <div className="relative z-10">
          <div className="startup-kicker"><span className="size-2 rounded-full bg-[#ff5938]" />Pre-execution safety for agentic finance</div>
          <h1 className="mt-7 max-w-[720px] text-[clamp(3.25rem,7vw,6.9rem)] font-semibold leading-[.89] tracking-[-.072em]">Stop the herd<br /><span className="text-[#ff5938]">before money moves.</span></h1>
          <p className="mt-8 max-w-xl text-lg leading-8 text-[#4e5852]">AI agents can make individually valid decisions that become dangerous together. HerdBrake sees the batch, holds it before signing, and gives treasury teams a controlled path forward.</p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row"><button onClick={onLaunch} className="startup-button startup-button-accent">Run the live scenario <ArrowRight /></button><a href="#platform" className="startup-button startup-button-light">See how it works</a></div>
          <p className="mt-5 flex items-center gap-2 text-xs text-[#69736d]"><ShieldCheck className="size-4" />Synthetic funds · deterministic controls · human authorization</p>
        </div>
    ...
    <section id="problem" className="startup-section">
      <div className="startup-container">
        <SectionLead index="01" eyebrow="The blind spot" title="Your policy engine checks transactions. Who checks the system?" body="Traditional controls ask whether one agent is allowed to pay. HerdBrake asks the harder question: if every allowed action executes at once, does the organization remain safe?" />
        <div className="mt-20 grid border-y border-[#c9cec6] lg:grid-cols-3 lg:divide-x lg:divide-[#c9cec6]">
          <ProblemColumn number="01" title="Individually compliant" body="Each agent stays within its own limit, approved destination, and assigned authority." />
          <ProblemColumn number="02" title="Collectively correlated" body="A common model, signal, or shock pushes independent agents toward the same defensive move." />
          <ProblemColumn number="03" title="Operationally dangerous" body="The combined outflow breaches liquidity floors before legacy monitoring sees the pattern." accent />
        </div>
      </div>
    </section>
    ...
    <section id="platform" className="startup-section bg-[#e7e9e2]">
      <div className="startup-container">
        <SectionLead index="02" eyebrow="The platform" title="One control plane between intent and execution." body="HerdBrake turns emerging agent-herding research into an operational workflow that treasury and risk teams can use before deployment and during execution." />
        <div className="mt-16 overflow-hidden border border-[#bbc1b8] bg-[#f7f7f3]">
          <div className="grid lg:grid-cols-[320px_1fr]">
            <div className="border-b border-[#bbc1b8] p-6 lg:border-b-0 lg:border-r lg:p-8">
              <p className="text-xs font-semibold uppercase tracking-[.16em] text-[#78817b]">Decision sequence</p>
              <div className="mt-8 space-y-1">{['Observe signed intents', 'Measure fleet correlation', 'Hold unsafe aggregate', 'Release in safe stages'].map((text, index) => <div key={text} className={`flex items-center gap-4 border-l-2 px-4 py-4 text-sm ${index === 2 ? 'border-[#ff5938] bg-[#ff5938]/7 font-semibold text-[#111714]' : 'border-[#c6cbc3] text-[#59635d]'}`}><span className="font-mono text-[11px] text-[#909890]">0{index + 1}</span>{text}</div>)}</div>
            </div>
            <div className="p-6 sm:p-9 lg:p-12"><PlatformDiagram /><div className="mt-9 grid gap-8 border-t border-[#d1d5cd] pt-8 sm:grid-cols-3"><MiniFeature icon={Radar} title="Herd detection" body="Direction, destination and timing concentration." /><MiniFeature icon={Pause} title="Shared breaker" body="One deterministic decision across the batch." /><MiniFeature icon={TimerReset} title="Staged release" body="Prioritize essentials, then recompute risk." /></div></div>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="startup-section bg-[#111714] text-white">
      <div className="startup-container">
        <div className="grid gap-16 lg:grid-cols-[.8fr_1.2fr]"><div><span className="section-index border-white/20 text-white/60">03</span><p className="mt-8 text-xs font-semibold uppercase tracking-[.18em] text-[#ff8067]">Designed to intervene</p><h2 className="mt-4 text-4xl font-semibold leading-tight tracking-[-.045em] sm:text-6xl">Not another AI wallet.<br />Not another research dashboard.</h2></div><div className="self-end"><p className="max-w-xl text-lg leading-8 text-white/60">The product distinction is not “many agents.” It is the combination of pre-execution visibility, aggregate safety policy, and an intervention that changes whether funds can move.</p><div className="mt-10 grid border-y border-white/15 sm:grid-cols-3 sm:divide-x sm:divide-white/15"><DarkFeature label="Observe" value="Unsigned intent" /><DarkFeature label="Decide" value="Aggregate policy" /><DarkFeature label="Intervene" value="Hold + release" /></div></div></div>
      </div>
    </section>
    ...
    <section id="taiwan" className="startup-section">
      <div className="startup-container">
        ...
        <div className="mt-16 grid gap-px overflow-hidden border border-[#c9cec6] bg-[#c9cec6] md:grid-cols-2">
          ...
          <ReadinessItem icon={Cpu} title="AI governance evidence" body="Inventory models, preserve decision logs, test failure modes, and keep deterministic controls outside the model." />
          <ReadinessItem icon={LockKeyhole} title="Human accountability" body="RBAC, dual control, critical-supplier priority, and explicit authorization before any signing step." />
        </div>
        <div className="mt-8 flex flex-col justify-between gap-5 border-t border-[#c9cec6] pt-6 text-sm text-[#5d6660] lg:flex-row lg:items-center"><p className="max-w-3xl">Reference path: Taiwan FSC AI guidance for financial institutions and the FinTech Regulatory Sandbox framework. These are design inputs—not a claim of approval or certification.</p><div className="flex shrink-0 gap-5"><a className="startup-text-link" href="https://law.fsc.gov.tw/LawContent.aspx?id=GL003920" target="_blank" rel="noreferrer">AI guidance ↗</a><a className="startup-text-link" href="https://law.fsc.gov.tw/LawContent.aspx?id=GL002360" target="_blank" rel="noreferrer">Sandbox law ↗</a></div></div>
      </div>
    </section>
    ...
    <section className="startup-section bg-[#ff5938] text-[#111714]">
      <div className="startup-container grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.18em]">Live product demonstrator</p><h2 className="mt-5 max-w-4xl text-5xl font-semibold leading-[.97] tracking-[-.055em] sm:text-7xl">See thirty compliant agents become one unsafe batch.</h2></div><button onClick={onLaunch} className="startup-button startup-button-dark min-w-52">Open control plane <ArrowRight /></button></div>
    </section>

    <footer className="bg-[#111714] text-white"><div className="startup-container grid gap-12 py-14 md:grid-cols-[1fr_auto_auto]"><div><div className="flex items-center gap-2.5 text-lg font-semibold"><StartupMark inverted />HerdBrake</div><p className="mt-4 max-w-sm text-sm leading-6 text-white/50">Agentic Treasury Assurance for the moment between decision and execution.</p></div><div><p className="footer-label">Product</p><div className="footer-links"><button onClick={onLaunch}>Live demo</button><a href="#platform">Platform</a><a href="#taiwan">Taiwan readiness</a></div></div><div><p className="footer-label">Status</p><div className="footer-links"><span>Hackathon demonstrator</span><span>Simulation only</span><span>Human approval required</span></div></div></div><div className="startup-container flex flex-col gap-2 border-t border-white/10 py-6 text-[11px] text-white/40 sm:flex-row sm:justify-between"><span>© 2026 HerdBrake Taiwan</span><span>Research prototype · Not a bank, custodian, or licensed payment institution</span></div></footer>
  </main>;
}

function HeroProductPreview({ onLaunch }: { onLaunch: () => void }) {
  const nodes = Array.from({ length: 30 }, (_, i) => i < 24);
  return <button onClick={onLaunch} className="product-preview group text-left" aria-label="開啟 HerdBrake 互動產品">
    <div className="preview-top"><div className="flex items-center gap-2"><span className="size-2 rounded-full bg-[#1e5d49]" /><span>HerdBrake Control</span></div><span>Batch FX-042</span></div>
    <div className="grid lg:grid-cols-[1fr_210px]"><div className="p-6 sm:p-8"><div className="flex items-end justify-between border-b border-[#d8dcd5] pb-5"><div><p className="preview-label">Directional agreement</p><p className="mt-2 text-4xl font-semibold tracking-[-.05em]">82%</p></div><span className="preview-status">HOLD</span></div><div className="mt-8"><div className="flex justify-between text-[10px] font-semibold uppercase tracking-[.12em] text-[#757e78]"><span>30 treasury agents</span><span>Pre-signature</span></div><div className="mt-5 grid grid-cols-10 gap-2">{nodes.map((danger, index) => <span key={index} className={`aspect-square rounded-[3px] border ${danger ? 'border-[#ff5938] bg-[#ff5938]' : 'border-[#8c9690] bg-transparent'}`} />)}</div></div><div className="mt-8 flex items-center justify-between text-xs text-[#606963]"><span>Projected buffer <strong className="ml-1 text-[#111714]">74%</strong></span><span className="flex items-center gap-1 font-semibold text-[#111714]">Inspect decision <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-1" /></span></div></div><div className="border-t border-[#d8dcd5] bg-[#eceee8] p-6 lg:border-l lg:border-t-0"><p className="preview-label">Reason code</p><p className="mt-2 font-mono text-sm font-semibold">HB-LIQ-003</p><div className="mt-8 space-y-4"><PreviewRow label="Individual policies" value="30 PASS" safe /><PreviewRow label="Same action" value="24 / 30" /><PreviewRow label="Outflow" value="US$8.4M" /><PreviewRow label="Execution" value="NOT SIGNED" /></div></div></div>
  </button>;
}

function StartupMark({ inverted = false }: { inverted?: boolean }) { return <span className={`grid size-8 place-items-center border ${inverted ? 'border-white/30' : 'border-[#111714]'}`}><span className={`h-3.5 w-1.5 border-x-2 ${inverted ? 'border-[#ff8067]' : 'border-[#ff5938]'}`} /></span>; }
function ProofStat({ value, label }: { value: string; label: string }) { return <div className="px-6 py-8 sm:px-9"><p className="font-mono text-2xl font-semibold text-[#ff8067]">{value}</p><p className="mt-1 text-sm text-white/55">{label}</p></div>; }
function SectionLead({ index, eyebrow, title, body }: { index: string; eyebrow: string; title: string; body: string }) { return <div className="grid gap-8 lg:grid-cols-[140px_1fr_1fr] lg:items-start"><span className="section-index">{index}</span><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[#ff5938]">{eyebrow}</p><h2 className="mt-4 text-4xl font-semibold leading-[1.03] tracking-[-.045em] sm:text-5xl">{title}</h2></div><p className="max-w-xl text-base leading-7 text-[#5a645e] lg:pt-8">{body}</p></div>; }
function ProblemColumn({ number, title, body, accent = false }: { number: string; title: string; body: string; accent?: boolean }) { return <div className={`px-0 py-8 lg:px-8 lg:py-10 ${accent ? 'text-[#c83a21]' : ''}`}><span className="font-mono text-xs opacity-55">{number}</span><h3 className="mt-12 text-2xl font-semibold tracking-[-.03em]">{title}</h3><p className={`mt-3 text-sm leading-6 ${accent ? 'text-[#9f402d]' : 'text-[#606963]'}`}>{body}</p></div>; }
function MiniFeature({ icon: Icon, title, body }: { icon: typeof Radar; title: string; body: string }) { return <div><Icon className="size-5 text-[#ff5938]" /><h3 className="mt-4 text-sm font-semibold">{title}</h3><p className="mt-2 text-xs leading-5 text-[#69736d]">{body}</p></div>; }
...
function validateStressInput(input: unknown): { scenarioId: ScenarioId; severity: number } { if (!input || typeof input !== 'object') throw new Error('Expected scenarioId and severity.'); const value = input as Record<string, unknown>; const allowed = scenarios.map((item) => item.id); if (typeof value.scenarioId !== 'string' || !allowed.includes(value.scenarioId as ScenarioId)) throw new Error('Unknown scenarioId.'); if (typeof value.severity !== 'number' || value.severity < 0.1 || value.severity > 1) throw new Error('Severity must be between 0.1 and 1.'); return { scenarioId: value.scenarioId as ScenarioId, severity: value.severity }; }
function validateReleaseInput(input: unknown) { if (!input || typeof input !== 'object') throw new Error('Expected release count.'); const count = (input as Record<string, unknown>).count; if (typeof count !== 'number' || !Number.isInteger(count) || count < 1 || count > 10) throw new Error('Count must be an integer from 1 to 10.'); return count; }
