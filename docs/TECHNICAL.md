...
每個代理只看自己部門四張發票和部門預算。模型只能選 PAY 或 DELAY，不能提供新的金額、收款帳號、付款指令或政策。重複／未知／漏掉的發票、其他動作、额外欄位、不完整回應都會被拒絕；上游失敗不建立批次，也不切換成偽造的 AI 結果。
...
- **身分與資料隔離**：正式环境由 Sites 驗證登入；每次讀寫均綁定 owner。跨帳號查詢回 404。公開部署不能直接暴露信任身分標頭的 Worker。
